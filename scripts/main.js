...
(function() {
  var arc, arcTween, breadcrumb, chart, classes, colors, getQueryParams, isMobile, partition, path, queryParams, tooltip, x, y;
  ...
  chart = {
    ...
    getDimensions: function() {
      ...
      } else if ((window.innerWidth - 360) < window.innerHeight && (window.innerWidth - 360) > 500) {
        return window.innerWidth - 360;
      } else if ((window.innerHeight - 60) > 500) {
    ...
    mainCircle: null,
    radius: function() {
      return Math.min(chart.getDimensions(), chart.getDimensions()) / 2;
    },
    init: function() {
      this.el = d3.select('svg').attr('width', chart.getDimensions()).attr('height', chart.getDimensions()).append('g').attr('class', 'main').attr('transform', 'translate(' + chart.getDimensions() / 2 + ',' + (chart.getDimensions() / 2) + ')').on('mouseleave', chart.onMouseleave);
    ...
    highlight: function(el) {
      if (el.classList.contains('current')) {
        ...
        d3.selectAll('.arc').transition().style('opacity', 0.6);
        return d3.select(el).transition().style('opacity', 1);
      }
    },
    highlightAll: function() {
      return d3.selectAll('.arc').transition().style('opacity', 1);
    },
    zoomIn: function(d) {
      chart.mainCircle = d;
  ...
  arc = d3.svg.arc().startAngle(function(d) {
    ...
    if (d === chart.mainCircle) {
      return Math.max(0, y(d.y));
    } else if (d.depth === chart.mainCircle.depth + 1) {
      return Math.max(100, y(d.y));
    } else {
      return Math.max(0, y(d.y));
    }
  }).outerRadius(function(d) {
    if (d === chart.mainCircle) {
      return Math.max(100, y(d.y + d.dy));
    } else {
      return Math.max(100, y(d.y + d.dy));
    }
  });
  ...
  breadcrumb = {
    ...
    create: function(node) {
      return this.el.select('ul').append('li').each(function() {
        ...
        return d3.select(this).append('h2').attr('class', 'breadcrumb--label').text(function(d) {
          if (node.name.length > 25) {
            return node.name.substring(0, 24) + '...';
          } else {
            return node.name;
          }
        });
  ...
  tooltip = {
    ...
    show: function(d) {
      ...
      tooltip.el.select('.tooltip--value').text(Math.round(d.values[0]).toLocaleString('fr') + ' €').style('color', colors.getColor(d.values[0]));
      return tooltip.el.select('.tooltip--find-out-more').attr('href', d.url);
    }
  };

  x = d3.scale.linear().range([0, 2 * Math.PI]);

  y = d3.scale.sqrt().range([0, chart.radius()]);
  ...
  getQueryParams = function() {
    var pair, param, params, query, queryFragment, _i, _len, _ref;
    params = {};
    query = window.location.search.substring(1);
    _ref = query.split('&');
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      queryFragment = _ref[_i];
  ...
  d3.json(queryParams.test_case_url, function(error, testCase) {
    var data;
    data = {
      scenarios: [
        {
    ...
    return d3.json('http://api.openfisca.fr/api/1/simulate').header("Content-Type", "application/json").post(JSON.stringify(data), function(error, root) {
      ...
      chart.init();
      path = chart.el.selectAll('path').data((partition.nodes(data)).filter(function(d) {
        return d.values[0] !== 0;
      ...
      }).on('click', chart.zoomIn).each(function(d) {
        d3.select(this.parentNode).append('svg:text').attr('class', 'main-circle--label').attr('y', '-20').text(function(d) {
          if (d.name.length > 25) {
        ...
        d3.select(this.parentNode).append('svg:text').classed('main-circle--value', true).classed('number', true).attr('y', '20').text(function(d) {
          return Math.round(d.values[0]).toLocaleString('fr') + ' €';
        });
        return d3.select(this.parentNode).filter(function(d) {
          return d.depth !== 0;
        }).append('image').classed('return', true).attr('xlink:href', 'images/icon-return.png').attr('width', '35').attr('height', '35').attr('x', '-17.5').attr('y', '35').on('click', function() {
