...
(function() {
  var arc, arcTween, breadcrumb, classes, colors, height, onClick, partition, path, radius, svg, tooltip, width, x, y;
  ...
  breadcrumb = {
    ...
    update: function(d) {
      return this.el.select('ul').append('li').each(function() {
        d3.select(this).append('span').attr('class', 'breadcrumb--value').style('background', colors.getColor(d.values[0]));
        return d3.select(this).append('h2').attr('class', 'breadcrumb--label').text(d.name);
      });
    }
  };
  ...
  d3.json('data/example.json', function(error, root) {
    ...
    path = svg.selectAll('path').data(partition.nodes(data)).enter().append('path').attr('data-name', function(d) {
      return d.name;
    ...
    }).on('click', onClick).on('mouseover', tooltip.show).on('mousemove', tooltip.update).on('mouseout', tooltip.hide);
  });
