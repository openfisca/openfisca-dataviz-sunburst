...
(function() {
  var arc, arcTween, breadcrumb, classes, colors, height, heightRoot, mouseLeave, mouseOver, partition, path, radius, svg, tooltip, width, widthRoot, x, y, zoomIn;
  ...
  zoomIn = function(d) {
    ...
    d3.selectAll('path.' + elClass).style('opacity', '0');
  ...
  mouseLeave = function(d) {
    ...
    return d3.selectAll('path').transition().duration(750).style('opacity', 1).each('end', function() {
      return d3.select(this).on('mouseover', mouseOver);
    });
  };
  ...
  d3.json('data/example.json', function(error, root) {
    ...
    }).on('click', zoomIn).on('mouseover', mouseOver).each(function(d) {
      if (d.depth === 0) {
